...
const AddProduct = () => {
  ...
  const { data: categories = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://used-products-resale-server.vercel.app/categories"
      );
    ...
    fetch(`https://api.imgbb.com/1/upload?key=${imageHostKey}`, {
      ...
      .then((imgData) => {
        if (imgData.success) {
          ...
          fetch(
            `https://used-products-resale-server.vercel.app/product?email=${user?.email}`,
            {
              method: "POST",
              headers: {
  ...
  return (
    <div className="mt-10">
      <h2 className="text-3xl text-center font-semibold underline mb-5">
        Add Product
      </h2>
      <form onSubmit={handleSubmit(handleAddProduct)} className="px-10">
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          <div>
            ...
            <input
              ...
              className="border border-indigo-500 focus:outline-1 focus:outline-indigo-600  w-full rounded-md px-4 py-1"
              required
            />
          </div>
          ...
          <div>
            ...
            <input
              type="file"
              {...register("image")}
              className="file-input file-input-sm file-input-bordered file-input-black w-full "
            />
          ...
          <div>
            <label className="label text-sm font-semibold">
              <span className="label-text">Add your address</span>
            </label>
            <input
              type="text"
              {...register("location")}
          ...
          <div>
            ...
            <input
              ...
              placeholder="How long have you been using it?"
              className="border border-indigo-500 focus:outline-1 focus:outline-indigo-600  w-full rounded-md px-4 py-1"
            />
            {errors.usedTime && (
              <p className="text-red-600 text-xs">
        ...
        <div className="mt-5">
          <textarea
            ...
            className="textarea border-indigo-500 focus:outline-1 focus:outline-indigo-600 w-full text-lg placeholder:italic"
            placeholder="Product Description"
          ></textarea>
        </div>

        <button disabled={btnBlur} className="btn btn-primary btn-md mt-5">
          Add Now
        </button>
      </form>
    </div>
