...
const ReportedProducts = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://used-products-resale-server.vercel.app/reportedProduct?email=${user?.email}`,
        {
          headers: {
  ...
  const handleReportProductDelete = (id) => {
    ...
    if (permission) {
      fetch(
        `https://used-products-resale-server.vercel.app/reportProduct/${id}?email=${user?.email}`,
        {
          method: "DELETE",
          headers: {
            authorization: `Bearer ${localStorage.getItem("accessToken")}`,
